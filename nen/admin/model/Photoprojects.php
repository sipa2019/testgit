<?php
Class PhotoProjectsClass extends Db_Base {

    protected $_nameDB 		= 'ori_photo_projects';
	protected $_projectsDB 	= 'ori_projects_translation';
	...
	*/
    public function baseSelect ()
    {
		$select = "SELECT ".$this->_nameDB.".*, ".$this->_projectsDB.".title as title 
				...
				ON ".$this->_nameDB.".projects_id = ".$this->_projectsDB.".projects_id 
				";
				//var_dump($select);
		return $select;
	}
	...
	*/	
    public function paginatorCat ( $param = null, $page = 1, $limit = 20, $lang ) {
		...
		if(isset($_GET['parent_id'])){
			...
			$select 	.= " WHERE ".$this->_projectsDB.".lang = '".$lang."' AND ".$this->_projectsDB.".projects_id = '".$category."'";

		}else{
			$select 	.= " WHERE ".$this->_projectsDB.".lang = '".$lang."'";	
		}
